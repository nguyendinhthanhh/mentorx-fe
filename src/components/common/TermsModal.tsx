    ...
    <Dialog.Root open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-50 w-full max-w-2xl translate-x-[-50%] translate-y-[-50%] p-4 focus:outline-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%]">
          <div className="relative overflow-hidden rounded-[24px] bg-white dark:bg-slate-950 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/30 px-6 py-4">
              <div className="flex items-center gap-2">
                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-indigo-100 text-indigo-600 dark:text-indigo-500">
              ...
              <Dialog.Close asChild>
                <button className="rounded-full p-2 text-slate-400 transition hover:bg-slate-100 hover:text-slate-600 dark:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20">
                  <X className="h-5 w-5" />
                </button>
              </Dialog.Close>
            </div>

            <div className="max-h-[60vh] overflow-y-auto px-6 py-6 text-sm text-slate-600 dark:text-slate-400">
              <div className="space-y-6">
                <section>
                  <h3 className="mb-2 text-base font-bold text-slate-900 dark:text-slate-100">1. Chấp nhận Điều khoản</h3>
                  <p>
                ...
                <section>
                  <h3 className="mb-2 text-base font-bold text-slate-900 dark:text-slate-100">2. Mô tả Dịch vụ</h3>
                  <p>
                ...
                <section>
                  <h3 className="mb-2 text-base font-bold text-slate-900 dark:text-slate-100">3. Quỹ đảm bảo (Escrow) và Thanh toán</h3>
                  <p>
                ...
                <section>
                  <h3 className="mb-2 text-base font-bold text-slate-900 dark:text-slate-100">4. Chính sách Bảo mật</h3>
                  <p>
            ...
            <div className="border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/30 px-6 py-4 flex justify-end">
              <button
                onClick={onClose}
