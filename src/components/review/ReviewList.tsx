...
export default function ReviewList({ targetType, targetId, summary }: Props) {
  ...
  if (isError) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 py-12 text-center dark:border-slate-700 dark:bg-slate-900/50">
        ...
        <p className="font-semibold text-slate-700 dark:text-slate-200">{t('reviews.loadError')}</p>
        <button
          type="button"
          onClick={() => refetch()}
          className="mt-4 rounded-xl border border-emerald-200 bg-white px-4 py-2 text-sm font-semibold text-emerald-800 hover:bg-emerald-50 dark:border-emerald-500/25 dark:bg-slate-900 dark:text-emerald-200"
        >
  ...
  if (totalReviews === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 py-14 text-center dark:border-slate-700 dark:bg-slate-900/50">
        <MessageCircle className="mx-auto mb-3 h-10 w-10 text-slate-300 dark:text-slate-600" />
        <p className="font-semibold text-slate-700 dark:text-slate-200">{t('reviews.emptyTitle')}</p>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{t('reviews.emptyDescription')}</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-6 rounded-[20px] border border-slate-200 bg-white p-5 sm:grid-cols-[180px_minmax(0,1fr)] sm:p-6 dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col justify-center border-b border-slate-200 pb-5 sm:border-b-0 sm:border-r sm:pb-0 sm:pr-6 dark:border-slate-800">
        ...
        <div>
          ...
          {ratingDist.map(d => (
            ...
            >
              <span className={`w-9 text-xs font-bold ${ratingFilter === d.stars ? 'text-emerald-700 dark:text-emerald-300' : 'text-slate-500'}`}>
                {d.stars} ★
              </span>
      ...
      {reviews.length > 0 ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 py-10 text-center dark:border-slate-700 dark:bg-slate-900/50">
          <p className="font-semibold text-slate-700 dark:text-slate-200">{t('reviews.noMatches')}</p>
          <button
            type="button"
            onClick={() => selectRating(null)}
