...
export default function UserPublicProfilePage() {
  ...
  if (!userProfile) {
    return (
      <main className="min-h-[100dvh] bg-slate-50 dark:bg-slate-900/50 px-4 py-16 text-slate-950 dark:text-slate-100 sm:px-6">
        <div className="mx-auto max-w-md text-center">
          <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-slate-200 text-slate-500 dark:text-slate-400">
            <User className="h-5 w-5" />
          </div>
          <h1 className="mt-5 text-xl font-semibold">{t('user.publicProfile.notFoundTitle')}</h1>
          <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">{t('user.publicProfile.notFoundDescription')}</p>
        </div>
      </main>
    )
  }
  ...
  return (
    <main className="min-h-[100dvh] bg-slate-50 dark:bg-slate-900/50 pb-16 text-slate-950 dark:text-slate-100 dark:bg-slate-950 dark:text-white">
      ...
      <div className="mx-auto mt-8 max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_280px] lg:gap-16">
          <section className="min-w-0">
            <div className="flex flex-col gap-5 border-b border-slate-200 dark:border-slate-800 pb-8 sm:flex-row sm:items-end sm:gap-6 dark:border-slate-800">
              ...
              <div className="min-w-0 pb-1">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {userProfile.emailVerified && <BadgeCheck className="h-5 w-5 shrink-0 text-emerald-600 dark:text-emerald-500" aria-label={t('user.publicProfile.verified')} />}
                </div>
                <p className="mt-2 text-sm font-medium text-slate-500 dark:text-slate-400 dark:text-slate-400">{t('user.publicProfile.member')}</p>
              </div>
            </div>
            ...
            <section className="mt-10 max-w-3xl">
              ...
              {userProfile.bio ? (
                <p className="mt-3 whitespace-pre-wrap text-[15px] leading-7 text-slate-700 dark:text-slate-300 dark:text-slate-300">{userProfile.bio}</p>
              ) : (
                <p className="mt-3 text-[15px] leading-7 text-slate-500 dark:text-slate-400 dark:text-slate-400">{t('user.publicProfile.aboutEmpty')}</p>
              )}
            </section>
          </section>

          <aside className="border-t border-slate-200 dark:border-slate-800 pt-6 lg:border-t-0 lg:border-l lg:pl-8 lg:pt-1 dark:border-slate-800">
            ...
            <dl className="mt-5 space-y-5">
              ...
              <ProfileFact
                ...
                valueClassName={userProfile.emailVerified ? 'text-emerald-700 dark:text-emerald-400 dark:text-emerald-400' : undefined}
              />
              <ProfileFact
...
function ProfileFact({
  ...
  return (
    <div className="flex gap-3">
      ...
      <div>
        <dt className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">{label}</dt>
...
function ProfileSkeleton() {
  return (
    <main className="min-h-[100dvh] bg-slate-50 dark:bg-slate-900/50 px-4 py-12 dark:bg-slate-950 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl animate-pulse">
        ...
        <div className="mt-12 grid gap-10 lg:grid-cols-[minmax(0,1fr)_280px] lg:gap-16">
          <div>
            <div className="flex items-end gap-6 border-b border-slate-200 dark:border-slate-800 pb-8 dark:border-slate-800">
          ...
          <div className="space-y-5 border-t border-slate-200 dark:border-slate-800 pt-6 dark:border-slate-800 lg:border-l lg:pl-8 lg:pt-1"><div className="h-5 w-28 rounded bg-slate-200 dark:bg-slate-800" />{[0, 1, 2].map((item) => <div key={item} className="h-10 rounded bg-slate-200 dark:bg-slate-800" />)}</div>
        </div>
      </div>
    </main>
  )
