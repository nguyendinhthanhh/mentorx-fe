...
export default function MentorProjectsPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Premium Tab Bar */}
      <div className="flex items-center gap-1.5 rounded-2xl bg-slate-100/80 p-1.5 w-fit shadow-sm border border-slate-200/60">
        {projectTabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`group relative inline-flex h-11 items-center gap-2.5 rounded-xl px-5 text-sm font-bold transition-all duration-300 ${
                isActive
                  ? 'bg-white text-slate-900 shadow-md shadow-slate-200/60 ring-1 ring-inset ring-slate-200/50'
                  : 'text-slate-500 hover:text-slate-700 hover:bg-white/50'
              }`}
            >
              <Icon className={`h-4 w-4 transition-colors ${isActive ? 'text-emerald-600' : 'text-slate-400 group-hover:text-slate-500'}`} />
              <span>{tab.label}</span>
              {isActive && (
