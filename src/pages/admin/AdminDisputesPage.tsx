    ...
    <div className="space-y-6 pb-16">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="flex items-center gap-3 text-2xl font-bold text-slate-950 dark:text-white">
            <Scale className="h-6 w-6 text-emerald-600 dark:text-emerald-500" />
            {t('admin.disputes.title')}
          </h1>
          <p className="mt-1 max-w-3xl text-sm text-slate-600 dark:text-slate-400 dark:text-slate-400">{t('admin.disputes.subtitle')}</p>
        </div>
        <button type="button" onClick={() => queue.refetch()} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-slate-300 dark:border-slate-700 px-4 text-sm font-semibold dark:border-slate-700">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
        <section className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          {queue.isLoading ? (
            <div className="p-8 text-center text-sm text-slate-500 dark:text-slate-400">{t('common.loading')}</div>
          ) : queue.isError ? (
            <div className="p-8 text-center text-sm text-rose-600">{t('admin.disputes.loadFailed')}</div>
          ) : (queue.data?.content.length ?? 0) === 0 ? (
            <div className="p-8 text-center text-sm text-slate-500 dark:text-slate-400">{t('admin.disputes.empty')}</div>
          ) : (
              ...
              <table className="w-full min-w-[760px] text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                  {queue.data!.content.map((item) => (
                    <tr key={item.id} onClick={() => setSelected(item)} className={`cursor-pointer hover:bg-slate-50 dark:bg-slate-900/50 dark:hover:bg-slate-800/50 ${selected?.id === item.id ? 'bg-emerald-50 dark:bg-emerald-900/30 dark:bg-emerald-950/20' : ''}`}>
                      <td className="px-4 py-3"><p className="font-semibold">{item.title}</p><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{formatDateTime(item.createdAt)}</p></td>
                      ...
                      <td className="px-4 py-3"><span className="rounded-full bg-amber-100 dark:bg-amber-900/50 px-2.5 py-1 text-xs font-semibold text-amber-800 dark:text-amber-200">{item.status}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="flex items-center justify-end gap-2 border-t border-slate-200 dark:border-slate-800 p-4 dark:border-slate-800">
            ...
            <span className="text-sm text-slate-500 dark:text-slate-400">{page + 1} / {Math.max(queue.data?.totalPages ?? 1, 1)}</span>
        ...
        <aside className="h-fit rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-5 shadow-sm xl:sticky xl:top-6 dark:border-slate-800 dark:bg-slate-900">
          {!selected ? <p className="py-16 text-center text-sm text-slate-500 dark:text-slate-400">{t('admin.disputes.select')}</p> : (
            <div className="space-y-5">
              <div><p className="text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">{selected.disputeCategory}</p><h2 className="mt-1 text-lg font-bold">{selected.title}</h2><p className="mt-2 whitespace-pre-wrap text-sm text-slate-600 dark:text-slate-400 dark:text-slate-300">{selected.description}</p></div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button type="button" disabled={supportMutation.isLoading} onClick={() => supportMutation.mutate(selected.initiatorId)} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-slate-300 dark:border-slate-700 px-3 text-xs font-semibold dark:border-slate-700"><MessageSquare className="h-4 w-4" />{t('admin.disputes.messageInitiator')}</button>
                <button type="button" disabled={supportMutation.isLoading} onClick={() => supportMutation.mutate(selected.respondentId)} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-slate-300 dark:border-slate-700 px-3 text-xs font-semibold dark:border-slate-700"><MessageSquare className="h-4 w-4" />{t('admin.disputes.messageRespondent')}</button>
              </div>
              {selected.respondentResponse && <div className="rounded-xl bg-slate-50 dark:bg-slate-900/50 p-3 text-sm dark:bg-slate-800"><p className="mb-1 font-semibold">{t('admin.disputes.response')}</p>{selected.respondentResponse}</div>}
              {(selected.evidenceUrls?.length ?? 0) > 0 && <div><p className="mb-2 text-sm font-semibold">{t('admin.disputes.evidence')}</p><ul className="space-y-1">{selected.evidenceUrls!.map((url) => <li key={url}><a href={url} target="_blank" rel="noreferrer" className="break-all text-sm text-emerald-600 dark:text-emerald-500 underline">{url}</a></li>)}</ul></div>}
              {selected.status === 'RESOLVED' ? (
                <div className="rounded-xl bg-emerald-50 dark:bg-emerald-900/30 p-4 text-sm text-emerald-900 dark:text-emerald-100 dark:bg-emerald-950/20 dark:text-emerald-200"><p className="font-semibold">{t('admin.disputes.resolved')}</p><p className="mt-1">{selected.resolutionDetails}</p>{selected.refundAmountMxc != null && <p className="mt-2 font-semibold">{t('admin.disputes.refund')}: {formatCurrency(selected.refundAmountMxc)}</p>}</div>
              ) : !isAdmin(user) ? (
                <div className="flex gap-2 rounded-xl bg-amber-50 dark:bg-amber-900/30 p-4 text-sm text-amber-900 dark:text-amber-100 dark:bg-amber-950/20 dark:text-amber-200"><AlertTriangle className="h-5 w-5 shrink-0" />{t('admin.disputes.moderatorReadOnly')}</div>
              ) : (
                <div className="space-y-3 border-t border-slate-200 dark:border-slate-800 pt-4 dark:border-slate-800">
                  <label className="block text-sm font-semibold">{t('admin.disputes.outcome')}<select value={outcome} onChange={(event) => setOutcome(event.target.value as DisputeOutcome)} className="mt-2 h-11 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 px-3 dark:border-slate-700 dark:bg-slate-950">{OUTCOMES.map((value) => <option key={value} value={value}>{t(`admin.disputes.outcome.${value}`)}</option>)}</select></label>
                  {needsRefundAmount(outcome) && <label className="block text-sm font-semibold">{t('admin.disputes.refund')}<input type="number" min="0.01" step="0.01" value={refundAmount} onChange={(event) => setRefundAmount(event.target.value)} className="mt-2 h-11 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 px-3 dark:border-slate-700 dark:bg-slate-950" /></label>}
                  <label className="block text-sm font-semibold">{t('admin.disputes.resolutionDetails')}<textarea value={resolutionDetails} onChange={(event) => setResolutionDetails(event.target.value)} maxLength={2000} className="mt-2 min-h-28 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 p-3 dark:border-slate-700 dark:bg-slate-950" /></label>
...
function CaseDetail({ label, value }: { label: string; value: string }) {
  return <div><dt className="text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">{label}</dt><dd className="mt-1 break-words">{value}</dd></div>
}
