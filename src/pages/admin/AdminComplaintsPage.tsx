...
function priorityClass(bucket: PriorityBucket): string {
  switch (bucket) {
    ...
    case 'high':
      return 'bg-rose-50 text-rose-600 dark:bg-rose-900/20 dark:text-rose-400'
    case 'medium':
      return 'bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:bg-amber-900/20 dark:text-amber-400'
    case 'low':
      return 'bg-slate-50 dark:bg-slate-900/50 text-slate-600 dark:text-slate-400 dark:bg-slate-800 dark:text-slate-400'
    default:
...
function statusClass(status: ComplaintStatus): string {
  switch (status) {
    ...
    case ComplaintStatus.RESOLVED:
      return 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-500 dark:bg-emerald-900/20 dark:text-emerald-400'
    case ComplaintStatus.CLOSED:
      return 'bg-slate-50 dark:bg-slate-900/50 text-slate-600 dark:text-slate-400 dark:bg-slate-800 dark:text-slate-400'
    case ComplaintStatus.WITHDRAWN:
    case ComplaintStatus.EXPIRED:
      return 'bg-gray-50 dark:bg-gray-900/50 text-gray-500 dark:bg-gray-800 dark:text-gray-400'
    default:
      return 'bg-gray-50 dark:bg-gray-900/50 text-gray-600 dark:text-gray-400'
  }
}
...
function priorityIconColor(bucket: PriorityBucket): string {
  ...
  if (bucket === 'medium') {
    return 'bg-amber-50 dark:bg-amber-900/30 text-amber-600 border-amber-100 dark:border-amber-900/30'
  }
  return 'bg-slate-50 dark:bg-slate-900/50 text-slate-500 dark:text-slate-400 border-slate-100 dark:border-slate-800 dark:border-slate-700'
}
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 bg-white dark:bg-slate-950/50 dark:bg-slate-800/50 px-4 py-2 rounded-xl border border-white/50 dark:border-slate-700/50 shadow-sm">
          {t('admin.complaints.queue')}
        </p>
      </div>

      <div className="rounded-[2.5rem] border border-white/50 bg-white dark:bg-slate-950/70 shadow-xl shadow-slate-200/40 backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/70 dark:shadow-none overflow-hidden transition-all">
        <div className="flex flex-col gap-4 border-b border-slate-100 dark:border-slate-800/50 bg-slate-50 p-6 dark:border-slate-800/50 dark:bg-slate-800/30 sm:flex-row sm:items-center">
          <div className="relative flex-1 group">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-slate-200 dark:border-slate-800/60 bg-white dark:bg-slate-950/50 py-3.5 pl-14 pr-4 text-sm font-bold text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:border-emerald-500/30 focus:bg-white dark:bg-slate-950 focus:outline-none focus:ring-4 focus:ring-emerald-500/10 dark:border-slate-700/60 dark:bg-slate-800/50 dark:text-white dark:placeholder:text-slate-500 dark:text-slate-400 dark:focus:bg-slate-800 shadow-sm hover:border-slate-300 dark:border-slate-700 dark:hover:border-slate-600 transition-all"
            />
          </div>
          <select
            value={statusFilter}
            ...
            className="w-full cursor-pointer appearance-none rounded-2xl border border-slate-200 dark:border-slate-800/60 bg-white dark:bg-slate-950/50 px-6 py-3.5 text-sm font-bold text-slate-600 dark:text-slate-400 shadow-sm outline-none transition-all hover:border-slate-300 dark:border-slate-700 focus:border-emerald-500/30 focus:bg-white dark:bg-slate-950 focus:ring-4 focus:ring-emerald-500/10 dark:border-slate-700/60 dark:bg-slate-800/50 dark:text-slate-300 dark:hover:border-slate-600 dark:focus:bg-slate-800 sm:w-auto sm:min-w-[200px]"
        ...
        {isError ? (
          <div className="flex flex-col items-center gap-4 px-6 py-16 text-center">
            ...
            <div>
              <p className="text-sm font-black text-gray-900 dark:text-gray-100 dark:text-white">
          ...
          <div className="flex flex-col items-center gap-3 px-6 py-16 text-center">
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gray-50 dark:bg-gray-900/50 text-gray-400 dark:bg-gray-800 dark:text-gray-500">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-800/50 bg-slate-50 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:border-slate-800/50 dark:bg-slate-800/30 dark:text-slate-500">
        ...
        <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-800/50 px-8 py-6 bg-slate-50 dark:border-slate-800/50 dark:bg-slate-800/20">
          ...
          <div className="flex gap-3">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 dark:border-slate-800/60 bg-white dark:bg-slate-950/50 px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 transition-all hover:bg-white dark:bg-slate-950 hover:text-emerald-600 dark:text-emerald-500 hover:shadow-sm hover:-translate-y-0.5 hover:border-emerald-200 dark:border-emerald-800/50 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:translate-y-0 disabled:hover:shadow-none dark:border-slate-700/60 dark:bg-slate-800/50 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-emerald-400 dark:hover:border-emerald-800/50"
            >
...
function ComplaintRow({ complaint }: { complaint: ComplaintResponse }) {
  ...
  return (
    <tr className="group transition-colors hover:bg-slate-50 dark:bg-slate-900/30 dark:hover:bg-slate-800/80">
      <td className="px-8 py-5">
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 line-clamp-2 max-w-md text-xs font-medium text-slate-500 dark:text-slate-400 dark:text-slate-400 leading-relaxed">
      ...
      <td className="px-8 py-5 align-top">
        <p className="text-xs font-bold text-slate-700 dark:text-slate-300 dark:text-slate-300">
      ...
      <td className="px-8 py-5 align-top whitespace-nowrap">
        <span className="inline-flex rounded-xl bg-white dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800/60 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-400 dark:bg-slate-800/50 dark:border-slate-700/60 dark:text-slate-300 shadow-sm">
          {complaint.complaintCategory || '—'}
        </span>
      </td>
      <td className="px-8 py-5 align-top whitespace-nowrap">
        <span
          className={`inline-flex rounded-xl px-3 py-1.5 text-[10px] font-black uppercase tracking-widest shadow-sm ${priorityClass(bucket)} border ${bucket === 'urgent' || bucket === 'high' ? 'border-rose-200/60 dark:border-rose-800/30' : bucket === 'medium' ? 'border-amber-200  dark:border-amber-800/30' : 'border-slate-200 dark:border-slate-800/60 dark:border-slate-700/60'}`}
        >
      ...
      <td className="px-8 py-5 align-top text-xs font-medium text-slate-500 dark:text-slate-400 dark:text-slate-400 whitespace-nowrap">
        {formatDateTime(complaint.updatedAt)}
      </td>
      <td className="px-8 py-5 align-top text-right whitespace-nowrap">
        <Link
          to={`/admin/complaints/${complaint.id}`}
          className="inline-flex items-center gap-1 rounded-xl border border-slate-200 dark:border-slate-800/60 bg-white dark:bg-slate-950/50 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 transition-all hover:bg-white dark:bg-slate-950 hover:border-emerald-200 dark:border-emerald-800/50 hover:text-emerald-600 dark:text-emerald-500 hover:shadow-sm hover:-translate-y-0.5 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 dark:border-slate-700/60 dark:bg-slate-800/50 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:border-emerald-800/50 dark:hover:text-emerald-400"
        >
    ...
    <div className="divide-y divide-gray-50 dark:divide-gray-800">
      {Array.from({ length: 4 }).map((_, index) => (
        <div key={index} className="flex animate-pulse items-center gap-4 px-6 py-5">
          <div className="h-10 w-10 rounded-2xl bg-gray-100 dark:bg-gray-800 dark:bg-gray-800" />
          <div className="flex-1 space-y-2">
            <div className="h-3 w-1/2 rounded bg-gray-100 dark:bg-gray-800 dark:bg-gray-800" />
            <div className="h-3 w-1/3 rounded bg-gray-100 dark:bg-gray-800 dark:bg-gray-800" />
          </div>
        </div>
      ))}
    </div>
