...
export default function AdminReportsPage() {
  ...
  const getStatusColor = (status: ReportStatus) => {
    switch (status) {
      case ReportStatus.PENDING: return 'bg-amber-50 dark:bg-amber-900/30 text-amber-600 border border-amber-200 dark:border-amber-800/50 dark:bg-amber-900/20 dark:border-amber-800/30 dark:text-amber-400'
      case ReportStatus.UNDER_REVIEW: return 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-500 border border-emerald-200 dark:border-emerald-800/50 dark:bg-emerald-900/20 dark:border-emerald-800/30 dark:text-emerald-400'
      case ReportStatus.RESOLVED: return 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-500 border border-emerald-200 dark:border-emerald-800/50 dark:bg-emerald-900/20 dark:border-emerald-800/30 dark:text-emerald-400'
      case ReportStatus.DISMISSED: return 'bg-slate-50 dark:bg-slate-900/50 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-800 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-400'
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
        <div>
          <h1 className="bg-gradient-to-r from-slate-900 to-slate-600 bg-clip-text text-2xl font-extrabold tracking-tight text-transparent dark:from-white dark:to-slate-400 sm:text-3xl lg:text-4xl">Content Moderation</h1>
      ...
      <div className="rounded-2xl border border-white/50 bg-white dark:bg-slate-950/70 p-4 shadow-xl shadow-slate-200/40 backdrop-blur-xl transition-all dark:border-slate-800 dark:bg-slate-900/70 dark:shadow-none sm:rounded-[2.5rem] sm:p-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-6">
          ...
          <div className="hidden h-8 w-px bg-slate-200/60 dark:bg-slate-700/60 sm:block" />
      ...
      <div className="bg-white dark:bg-slate-950/70 dark:bg-slate-900/70 backdrop-blur-xl rounded-[2.5rem] border border-white/50 dark:border-slate-800 shadow-xl shadow-slate-200/40 dark:shadow-none overflow-hidden transition-all">
            ...
            <tbody className="divide-y divide-slate-100/50 dark:divide-slate-800/50">
              {isLoading ? (
                ...
                data?.content.map((report) => (
                  <tr key={report.id} className="group hover:bg-slate-50 dark:bg-slate-900/30 dark:hover:bg-slate-800/80 transition-colors">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center border shadow-sm group-hover:scale-105 transition-transform ${
                          report.priorityLevel > 3 ? 'bg-rose-50 text-rose-600 border-rose-200/60 dark:border-rose-800/30' : 'bg-amber-50 dark:bg-amber-900/30 text-amber-600 border-amber-200  dark:border-amber-800/30'
                        }`}>
                          {report.priorityLevel > 3 ? <ShieldAlert className="w-6 h-6" /> : <Flag className="w-6 h-6" />}
                        </div>
                        <div className="flex flex-col min-w-0">
                          <span className="text-sm font-bold text-slate-900 dark:text-white tracking-tight truncate max-w-[200px] group-hover:text-emerald-600 dark:text-emerald-500 dark:group-hover:text-emerald-400 transition-colors">{report.reason}</span>
                          <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mt-1">{report.reportCategory.replace(/_/g, ' ')}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-slate-700 dark:text-slate-300 dark:text-slate-300">{report.reporterName}</span>
                        <span className="text-[10px] font-medium text-slate-500 dark:text-slate-400 dark:text-slate-400 mt-1">{formatDateTime(report.createdAt)}</span>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-slate-700 dark:text-slate-300 dark:text-slate-300 flex items-center gap-2">
                    ...
                    <td className="px-8 py-6 text-right">
                      <div className="flex items-center justify-end gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-all duration-300">
                        <button className="p-3 rounded-xl bg-white dark:bg-slate-950/50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800/60 dark:border-slate-700/60 text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:text-emerald-500 hover:border-emerald-200 dark:border-emerald-800/50 dark:hover:border-emerald-800/50 hover:bg-white dark:bg-slate-950 transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5">
                          <Eye className="w-5 h-5" />
                        </button>
                        {report.status === ReportStatus.PENDING && (
                          <button 
                            onClick={() => assignMutation.mutate(report.id)}
                            className="p-3 rounded-xl bg-white dark:bg-slate-950/50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800/60 dark:border-slate-700/60 text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:text-emerald-500 hover:border-emerald-200 dark:border-emerald-800/50 dark:hover:border-emerald-800/50 hover:bg-white dark:bg-slate-950 transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5"
                            title="Assign to me"
                          >
                        ...
                        <button className="p-3 rounded-xl bg-white dark:bg-slate-950/50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800/60 dark:border-slate-700/60 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-slate-100 dark:hover:text-white hover:bg-white dark:bg-slate-950 transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5">
        ...
        {/* Pagination */}
        <div className="px-8 py-6 border-t border-slate-100 dark:border-slate-800/50 dark:border-slate-800/50 flex items-center justify-between bg-slate-50 dark:bg-slate-800/20">
          <p className="text-xs font-bold text-slate-400 dark:text-slate-500">
            Showing <span className="font-black text-slate-600 dark:text-slate-400 dark:text-slate-300">{data?.totalElements === 0 ? 0 : ((data?.number ?? 0) * (data?.size ?? 0)) + 1}</span> - <span className="font-black text-slate-600 dark:text-slate-400 dark:text-slate-300">{Math.min(((data?.number ?? 0) + 1) * (data?.size ?? 0), data?.totalElements ?? 0)}</span> of <span className="font-black text-slate-600 dark:text-slate-400 dark:text-slate-300">{data?.totalElements ?? 0}</span> reports
          </p>
          <div className="flex gap-3">
            <button 
              disabled={page === 0}
              onClick={() => setPage(p => p - 1)}
              className="p-3 rounded-xl border border-slate-200 dark:border-slate-800/60 bg-white dark:bg-slate-950/50 text-slate-500 dark:text-slate-400 transition-all hover:bg-white dark:bg-slate-950 hover:text-emerald-600 dark:text-emerald-500 hover:shadow-sm hover:-translate-y-0.5 hover:border-emerald-200 dark:border-emerald-800/50 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:translate-y-0 disabled:hover:shadow-none dark:border-slate-700/60 dark:bg-slate-800/50 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-emerald-400 dark:hover:border-emerald-800/50"
            >
